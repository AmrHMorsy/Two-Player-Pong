...
Events_Manager::Events_Manager(SDL_Renderer * renderer,Mix_Chunk * soundeffect, int procnum, int f_d1, int f_d2, int f_d3, int f_d4 )
{
    fd1 = f_d1 ;
    fd2 = f_d2 ;
    fd3 = f_d3 ;
...
void Events_Manager::Update_Ball_Position()
{
    ...
    if( ProcNum == 1 ){
        ...
        if( BallXPos <= -50 ){
            ...
            Right = false ;
            sleep(1) ;
            Send_Message(fd3,("1 "+ std::to_string(BallYPos) + " " + std::to_string(BounceAngle) + " " + std::to_string(1) + " ")) ;
        }
        if( BallXPos >= 650 ){
            Left = true ;
            Right = false ;
            BallInProc = 2 ;
            BallXPos = 649 ;
            Send_Message(fd3,("2 "+ std::to_string(BallYPos) + " " + std::to_string(BounceAngle) + " " + std::to_string(0) + " ")) ;
        }
    }
    else if( ProcNum == 2 ){
        ...
        if( BallXPos >= 650 ){
            ...
            Right = false ;
            sleep(1) ;
            Send_Message(fd3,("2 "+ std::to_string(BallYPos) + " " + std::to_string(BounceAngle) + " " + std::to_string(1) + " ")) ;
        }
        if( BallXPos <= 0 ){
            Left = false ;
            Right = true ;
            BallInProc = 1 ;
            BallXPos = 1 ;
            Send_Message(fd3,("1 "+ std::to_string(BallYPos) + " " + std::to_string(BounceAngle) + " " + std::to_string(0) + " ")) ;
        }
    }
